...
**先真的把这张图读进来再分析（Claude Code 用 Read 工具，Codex 用原生多模态）。绝不凭记忆或想象猜测图片内容。**
读不到（路径不存在）就停下来说明，不要编造。

## 输出

**只输出一个 JSON 对象到标准输出。不要代码块围栏，不要任何解释文字，不要写文件。**

字段和枚举值以这份 schema 为准，先读一遍：
{{SCHEMA_PATH}}
...
- `keywords`：内容层的具体名词，**中英并列都写**。单个汉字不会被检查，至少两个字。

**keywords 只放"换主体就不成立"的东西。** 每个词过一遍这个判定：

> 把主体换成一台咖啡机，这个词还成立吗？

- 「体重秤」→ 换成咖啡机就不成立 → **是内容**，写进 keywords ✓
- 「薄荷绿胶条」→ 咖啡机上没有 → **是内容** ✓
- 「渐变背景」→ 换成咖啡机照样是渐变背景 → **是风格**，不要写进 keywords ✗
- 「柔和侧光」「哑光质感」「大面积留白」→ 同理，都是风格 ✗

背景性质、光线、材质、构图、色调**一律不进 keywords**——它们属于风格层，写进来会让
style_dna 里正常的风格描述被误判成内容泄漏。keywords 里只该有画面上那些**具体的、
可以被整个替换掉的物件**。
...
色板取到能看出的主色/辅色/点缀色，`hex` 要真的从画面取色。
`ratio` 是各色占画面的比例，只列主色即可，合计不必等于 1（剩下的是不值得单列的杂色），但不能超过 1。
